...
ERAS = [('1980_1989', 1980, 1989), ('1990_1998', 1990, 1998),
        ('1999_2004', 1999, 2004), ('2005_2009', 2005, 2009), ('2010_2014', 2010, 2014),
        ('2015_2019', 2015, 2019), ('2020_2025', 2020, 2025)]

# Pre-1999 results, standings and playoff bracket per season, keyed by franchise
# lineage id. nflverse's games file starts at 1999, so the hydrated eras need a
# second source -- the same licensed one packages/data reads season stats from.
# Absent on a machine that has not hydrated those eras, which is not an error.
RESULTS = ROOT / 'data' / 'raw' / 'results' / 'results-1980-1998.json'

# PFR's bracket names, in nflverse's vocabulary.
ROUNDS = {'WildCard': 'WC', 'Division': 'DIV', 'ConfChamp': 'CON', 'SuperBowl': 'SB'}
...
def sentence(seasons: list) -> str:
    ...
    if won:
        # "XVI, XIX, XXIII and XXIV" -- four titles joined by four "and"s read
        # like a list a machine wrote, and San Francisco's eighties needed it.
        numerals = [roman(s['year'] - 1965) for s in won]
        if len(numerals) == 1:
            return f'Won Super Bowl {numerals[0]}.'
        titles = ', '.join(numerals[:-1]) + f' and {numerals[-1]}'
        return f'Won Super Bowls {titles}.'
    if lost:
        s = lost[0]
        if s['l'] == 0:
...
def load_pre1999(tally: dict) -> int:
    """Folds the hydrated seasons in, in the same shape the CSV path produces."""
    if not RESULTS.exists():
        return 0
    seasons = json.load(open(RESULTS))
    for year_key, season in seasons.items():
        year = int(season.get('year', year_key))
        for row in season['standings']:
            entry = tally[(row['fr'], year)]
            entry['w'] += row['w'] or 0
            entry['l'] += row['l'] or 0
            entry['t'] += row['t'] or 0
        for game in season['playoffs']:
            kind = ROUNDS.get(game['round'], game['round'])
            # Both sides played the round; only one of them won the last one.
            tally[(game['w'], year)]['post'].add(kind)
            tally[(game['l'], year)]['post'].add(kind)
            if kind == 'SB':
                tally[(game['w'], year)]['sb_won'] = True
    return len(seasons)
...
''')
    print(f'{OUT.relative_to(ROOT)}  {len(lines)} franchise-eras'
          + (f' (including {hydrated} hydrated seasons)' if hydrated else ''))
