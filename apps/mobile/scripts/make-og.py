#!/usr/bin/env python3
"""
Build the link preview card.

    /tmp/imgvenv/bin/python scripts/make-og.py

Every share of this game ends up as a URL somewhere — iMessage, a group chat,
a timeline. Until this existed the page carried no Open Graph tags at all, so
all of those rendered as the bare string "mutaaf.github.io" with no title, no
picture and no reason to tap. That is what the share sheet screenshot was
showing: not a bug in the share code, an empty <head>.

The card is 1200x630, the size every scraper crops to. It is drawn rather than
screenshotted because a screenshot of a dense UI is unreadable at the ~300px
these are actually displayed at.

Pillow is not a project dependency; this runs by hand when the brand changes.
"""
from pathlib import Path
from PIL import Image, ImageDraw, ImageFilter, ImageFont

ROOT = Path(__file__).resolve().parent.parent
FONTS = ROOT.parent.parent / 'node_modules' / '@expo-google-fonts'

W, H = 1200, 630
VOID = (6, 8, 15)
GOLD = (255, 180, 0)
GOLD_BRIGHT = (255, 209, 82)
TEXT = (242, 245, 250)
DIM = (154, 164, 184)


def font(family: str, style: str, size: int) -> ImageFont.FreeTypeFont:
    return ImageFont.truetype(str(FONTS / family / style / f'{family.capitalize()}_{style}.ttf'), size)


def background() -> Image.Image:
    card = Image.new('RGB', (W, H), VOID)
    draw = ImageDraw.Draw(card)

    # Yard lines, barely there — texture at full size, invisible at thumbnail
    # size, which is the right way round.
    for x in range(0, W, 60):
        draw.line([(x, 0), (x, H)], fill=(13, 17, 28), width=1)

    # A gold glow behind where the crest sits, blurred so it reads as light
    # rather than as a circle.
    glow = Image.new('RGB', (W, H), VOID)
    ImageDraw.Draw(glow).ellipse([-40, 40, 620, 700], fill=(58, 40, 4))
    card = Image.blend(card, glow.filter(ImageFilter.GaussianBlur(90)), 0.85)

    # A single red rule along the bottom: the broadcast palette's live colour.
    ImageDraw.Draw(card).rectangle([0, H - 8, W, H], fill=(213, 10, 10))
    return card


def main() -> None:
    card = background()

    crest = Image.open(ROOT / 'assets' / 'brand' / 'lockup.png').convert('RGBA')
    crest = crest.crop(crest.getbbox())
    scale = 470 / crest.size[0]
    crest = crest.resize((int(crest.size[0] * scale), int(crest.size[1] * scale)), Image.LANCZOS)
    card.paste(crest, (60, (H - crest.size[1]) // 2 - 10), crest)

    draw = ImageDraw.Draw(card)
    x = 580

    draw.text((x, 196), 'BUILD THE', font=font('rajdhani', '700Bold', 66), fill=TEXT)
    draw.text((x, 258), 'PERFECT ROSTER', font=font('rajdhani', '700Bold', 66), fill=GOLD_BRIGHT)

    draw.text((x, 344), 'Seven spins. Sixty years of pro football.',
              font=font('montserrat', '400Regular', 23), fill=DIM)
    draw.text((x, 380), 'One shot at an undefeated season.',
              font=font('montserrat', '400Regular', 23), fill=DIM)

    # The label, set the way the app sets labels: small, uppercase, tracked out.
    label = font('rajdhani', '600SemiBold', 22)
    tracked = ' '.join('18-0.CO')
    draw.text((x, 452), tracked, font=label, fill=GOLD)

    out = ROOT / 'public' / 'og.png'
    out.parent.mkdir(parents=True, exist_ok=True)
    card.save(out, optimize=True)
    print(f'{out.relative_to(ROOT.parent.parent)}  {card.size[0]}x{card.size[1]}  '
          f'{out.stat().st_size // 1024} KB')


if __name__ == '__main__':
    main()
